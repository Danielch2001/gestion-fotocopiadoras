...
const getFotocopiadorasDisponibles = async () => {
  const result = await pool.query("SELECT * FROM fotocopiadora WHERE estado = 'disponible'");
  return result.rows;
};

// 📌 Agregar una fotocopiadora
const addFotocopiadora = async (fotocopiadora) => {
  const { modelo, marca, estado, precio, descripcion, ubicacion, fecha_adquisicion, contador_impresiones } = fotocopiadora;
...
const deleteFotocopiadora = async (id) => {
  const result = await pool.query("DELETE FROM fotocopiadora WHERE idfotocopiadora = $1 RETURNING *", [id]);
  return result.rows[0];
};
